...
const resp = await fetch('https://sys.flyrbp.com/api2.php?scope=Booking&action=getFlights', {
  method: 'POST',
  headers: { Accept: 'application/json, text/plain, */*', 'Content-Type': `multipart/form-data; boundary=${boundary}`, Origin: 'https://flyrbp.com', Referer: 'https://flyrbp.com/', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36' },
  body,
});
const json = await resp.json();
const target = (json.data.hin ?? []).filter((f) => (f.ab_datum_zeit ?? '').startsWith('2026-08-15'));
for (const f of target) {
  console.log(JSON.stringify({ flugnr: f.flugnr, ab_zeit: f.ab_zeit, summe_hs: f.preise?.summe_hs, available: f.available }));
}
